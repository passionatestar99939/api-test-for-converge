...
app.use((err, req, res, next) => {
  if (req.originalMethod === "POST") {
    if (/\/data/.test(req.originalUrl)) {
      if (req.body.sensorId === undefined || req.body.time === undefined) {
        if (req.body.sensorId === undefined)
          console.log(`________ Error 400: sensorId is required.`);
        if (req.body.time === undefined)
          console.log(`________ Error 400: time is required.`);
        return res.json({ status: "error 400" });
      }
    } else if (/\/sensors/.test(req.originalUrl)) {
      if (req.body.id === undefined || req.body.threshold === undefined) {
        if (req.body.id === undefined)
          console.log(`________ Error 400: id is required.`);
        if (req.body.threshold === undefined)
          console.log(`________ Error 400: threshold is required.`);
        return res.json({ status: "error 400" });
      }
    }
  }
